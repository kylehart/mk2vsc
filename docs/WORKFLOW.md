...
3. File it immediately into the change folder (`00_baseline/`, see docs/CHANGE_CONTROL.md) and run
   `rvms info` to confirm the serials and the save timestamp.
...
3. A settings-only upload (what `rvms set` produces) does not reset the VE.Bus and does not
...
- `rvms diff 01_prepared/<file> 02_downloaded/<file>` reports ONLY BOOKKEEPING.
