    ...
    for serial, name, value in changes:
        ...
        if fld.id in GRID_CODE_LOCKED:
            raise WriteRefused(f"{fld.name} (setting {fld.id}) is part of the grid-code block or its validity words "
                               f"(81, 128, 129-189, 190, 191); mk2vsc never edits these and there is no override. "
                               f"A grid code reaches a device through VEConfigure, or by file only as a complete "
...
def set_bits(data: bytes, changes: Iterable[Tuple[Optional[str], object, int, bool]],
    ...
    for serial, name, bit, on in changes:
        fld = lookup(name)
        if fld.id in GRID_CODE_LOCKED:
            raise WriteRefused(f"{fld.name} (setting {fld.id}) is grid-code locked; no bit edits either")
        if fld.bits is None:
