...
FIELDS: List[Field] = [
    ...
    _f(8, "repeated_absorption_interval_min", "Repeated Absorption Interval", 1, "min", HIGH,
       "Interval between re-absorptions, in minutes (raw x 360, i.e. 6-hour steps).", f"{MK2}; schema scale 360, 1 to 180.",
       "4, 28", "mk2"),
    ...
    _vs_level(28, "vs_off_inverter_current_low_A", "vsoffIInvLow", "off when inverter current lower than (0.01 A)", "531", scale=100, unit="A"),
    ...
    _f(50, "aes_low_current_limit_A", "Low current limit for switching to AES", 100, "A", HIGH,
       "AC load current below which the inverter enters AES (search/low-power mode).", f"{MK2}; schema /100, 0.21 to 2.55 A, default 0.60.", "60", "mk2"),
    ...
    _f(59, "vs_udc_above_for_min", "vs2offUBatHigh (time)", 1, "min", HIGH, "Duration for the Udc-high condition, minutes (raw - 1).",
       f"{MK2}; schema offset -1; raw 2 = 1 min.", "0, 2", "mk2", lo=0, hi=254, raw_offset=-1),
    _f(60, "flags2", "Flags2", 1, "bitmask", HIGH, "Bit register: vs2offWhenAC1Available (bit 0), vs2Invert (bit 1), vsSetInverterPeriodTime (bit 2). Schema max 1022.",
    ...
    _f(62, "output_frequency_Hz", "vsInverterPeriodTime", 2500, "Hz", HIGH,
       "Inverter output frequency, stored as the period in 1/2500 ms: 41667 = 16.667 ms = 60 Hz. Applied when Flags2 bit 2 (vsSetInverterPeriodTime) is set.",
