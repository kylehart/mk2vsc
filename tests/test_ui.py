...
def test_every_placed_setting_is_a_known_identifier():
    for eprom, ui in UI_BY_EPROM.items():
        ...
        assert ui.source in ("rvsc-tools", "ours")
...
def test_every_placed_bit_is_a_flag_register_bit():
    for (sid, bit), ui in UI_BITS.items():
        ...
        assert ui.kind == "bool" or (sid, bit) == (0, 2)
...
def test_placement_counts():
    """62 numeric/enum settings (57 from talas9's 102-field table, 5 more from our Ignore-AC screenshot) and 32 bits placed,
    7 derived, 6 unplaced; every field with a placement is at least LOW."""
    assert len(UI_BY_EPROM) == 62 and len(UI_BITS) == 32 and len(DERIVED) == 7 and len(UNPLACED) == 6
    assert sum(1 for u in UI_BY_EPROM.values() if u.source == "ours") == 9
...
def test_units_agree_where_both_sides_state_one():
    ...
    for eprom, ui in UI_BY_EPROM.items():
        f = next(x for x in FIELDS if x.eprom == eprom)
        if ui.unit in ("V", "A", "Ah", "%", "s", "min") and f.unit:
