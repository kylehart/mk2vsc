...
def test_show_default_and_all_and_json(capsys):
    ...
    assert "vs_param52" not in out
    assert main(["show", BARE, "--all"]) == 0
    assert "vs_param52" in capsys.readouterr().out
...
def test_edit_refusals(tmp_path, capsys):
    ...
    assert main(["edit", str(src), "vs_param52=1000"]) == 1           # low confidence
...
def test_diff_validate_fields_census_history(capsys):
    ...
    out = capsys.readouterr().out
    assert "absorption" in out and "vs_param52" not in out
