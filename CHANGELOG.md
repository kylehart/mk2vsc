...
### Added
- **Inverter parity check** (`mk2vsc/parity.py`). The two blocks of a pair are compared on all 192
  registers, named or not, against Victron's documented per-unit exception list (grid code 81 and
  128-191, Virtual Switch, DC low shutdown). `check_parity()` / `check_parity_bytes()` RAISE
  `ParityMismatch` on a difference outside that list, so a caller must handle it or ask for the
  report form (`parity_report_bytes()`). Differences in settings that act on the shared battery
  (absorption, float, charge current, absorption timing, capacity, SoC bookkeeping) are classed as
  probable commissioning errors. New CLI command `mk2vsc parity FILE...` (exit 2 on mismatch);
  `mk2vsc show` now prints the parity report when the pair differs.
