...
* **Inverter parity check** (`mk2vsc/parity.py`). Every inverter block is compared on all 192 registers, named or not, against Victron's documented per-unit exception list (grid code 81 and 128-191, Virtual Switch, DC low shutdown). `check_parity()` / `check_parity_bytes()` RAISE `ParityMismatch` on a difference outside that list and `ParityNotComparable` when the file cannot be judged (fewer than two blocks, a short block, duplicate serials), so a caller must handle both or ask for the report form (`parity_report_bytes()`). Differences in settings that act on the shared battery are classed as probable commissioning errors. Written for N >= 2 inverters so three-phase files are checked rather than refused, but **only pairs are exercised against real hardware; N > 2 is untested and unsupported** (`docs/PARITY.md`). New CLI `mk2vsc parity FILE...` (exit 2 on mismatch, 1 when a file could not be read or compared); `mk2vsc show` prints the parity report when the units differ. Firmware is compared as well as settings (units must run the same version), and the block-length guard reserves the section checksum so a slightly short block cannot read checksum bytes as settings 190/191.
* Single-unit `.rvsc` and three-phase `.rvms` files. `fixtures/synthetic/`: a single-unit file (the byte prefix of a System A download), a three-unit and a six-unit three-phase file (one System A ESS block cloned with serials HQ0000A0003.., phase bytes 00/04/08, unit index, flags e8/e9/ea), built by `tools/gen_synthetic_fixtures.py` and checked against it; manifest origin `synthetic`, excluded from the corpus claim tests. `UnitBlock.phase_byte`, `unit_index`, `phase_label`, `phase_summary` decode +0x35/+0x37; `census`, `show` and the JSON print `phase L2, unit 1`. `has_assistant_flag` reads the flag's high nibble (e = records) instead of the pair values e4/e5. `firmware_version` and `schema.firmware_of_schema` return the low 24 bits of the firmware word; `firmware_word` and `firmware_word_high_byte` expose the rest and `census`/`show` say `(word high byte 0xc7)` when it is set; corpus output unchanged. `diagnose`: rules declare `min_units`; D2 and E2 do not run on a one-block file and the report carries `not_applicable {rule: why}` (files[] in the `report_version` 1 JSON; the `unverified_format` key and the `.rvsc` note are gone: the extension changes nothing). `tools/validate_dir.py <dir>`: the structural checks over a directory of files that cannot enter the repository, printing aggregate counts only (one of its checks is `records_imply_assistant_flag`: a block carrying assistant records always has flag high nibble `e`, and the converse does not hold, since stub, container and empty areas sit behind an `e` nibble too). An unrecognised `Mk2vscInfo` version string is bucketed rather than printed, the phase-model check requires a contiguous unique unit-index set, and `--strict` makes a value finding (alignment, census) set the exit status as a CI caller wants; docs/QA.md carries the table for ten files from other systems (1 to 6 units, four firmware families, format 1.30 to 1.33). docs/FORMAT.md 3.1.1 (phase/unit bytes per topology) and 3.3 (the firmware word's high byte, one file); README limits rewritten (covered: VE.Bus inverter/chargers behind a GX, `.rvsc` and `.rvms`, Venus 3.60 `/data/conf/` backups; not covered: VictronConnect and VEConfigure `.vsc`, MK3-only installs); the two offline checks (VEConfigure 3 in a VM; `mk2vsc -L -f` on a GX) in docs/QA.md and CONTRIBUTING.md; docs/donate.md.
* `SectionTooShort` (a `RvmsParseError`): a file whose sections and checksums are fine but whose `BareSettingInfo` or `BareSettingData` payload is shorter than the layout mk2vsc reads is refused by name, with the bytes found, the bytes needed and the record count a short schema holds, by `show`, `census`, `check`, `verify`, `diagnose`, `history` and `mk2vsc.load`; `validate` stays container and checksums only. `mk2vsc.units.check_layout(file)` runs the check; `schema.parse_schema` raises it. The 192-record schema length is not generalised (evidence and TODO in `mk2vsc/schema.py`). A one-block file cut from a corpus fixture is a byte prefix of it and parses, validates, aligns and round-trips (tests/test_short_files.py).
