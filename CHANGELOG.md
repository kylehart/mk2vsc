...
* `mk2vsc set-bits FILE FIELD:BIT=0|1 [...]` and `api.Config.set_bit()`: the CLI and library entry points for the bit writer that `writer.set_bits` already provided. Same guards (settable mask, qualified-bit list, `--allow-unqualified`, grid-code lock, byte-diff proof), `--serial` for one inverter, and a closing note that a flag-register write has never been measured on a live system so the dialog decides whether it was interruption-free.
